...
void Lab3_Vis2D::Init()
{
    ...
    glm::vec3 corner = glm::vec3(1, 0.009, 0);
    length = 0.99f;

    //Mesh* square1 = object2D::CreateSquare("square1", corner, length, glm::vec3(0.6, 0.6, 0));
...
void Lab3_Vis2D::Update(float deltaTimeSeconds)
{
    ...
    SetViewportArea(viewSpace, glm::vec3(0.6f, 0.4f, 0.2f), true);
...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_W)) logicSpace.y -= deltaTime;
    if (window->KeyHold(GLFW_KEY_S)) logicSpace.y += deltaTime;

    if (window->KeyHold(GLFW_KEY_A)) logicSpace.x += deltaTime;
    if (window->KeyHold(GLFW_KEY_D)) logicSpace.x -= deltaTime;


    // TODO(student): Zoom in and zoom out logic window with Z and X

    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.x -= 0.005f * logicSpace.width;
        logicSpace.y -= 0.005f * logicSpace.height;
